...
impl EGraph {
    ...
    fn to_graphviz(&self) -> Graph {
        // 1. Group nodes by class and create mapping from each node ID to its class
        ...
        for (class_id, nodes) in class_nodes {
            ...
            if let Some(typ) = typ {
                // Lookup color index for typ, or assign a new one, wrapping around
                let next_color = typ_colors.len() % N_COLORS + 1;
                let color = typ_colors.entry(typ).or_insert(next_color);
                inner_stmts.push(stmt!(attr!("fillcolor", color)));
            }
